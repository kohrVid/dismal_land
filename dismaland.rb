...
loc_id = 19
while cmd != "q"
  next_loc = locations[loc_id - 1][:directions][cmd.to_sym]
  loc_id = next_loc.to_i if next_loc != nil
  location = locations[loc_id - 1]
  puts "You're at the \e[#32m#{location[:name]}\e[0m"
  puts location[:description] if location[:description] != ""
  puts "------------"
  puts "From here, you can either press Q to quit, or go to:"
  location[:directions].each do |k, v|
    puts "#{k.to_s.upcase}: #{locations[v - 1][:name]}"
  end
